...
double Program::predict(const std::vector<double>& features) {
    execute(features);
    return registers[0];
}
...
std::vector<uint16_t> Program::__getstate__() const {
    // Return only the instructions for pickling
    return instructions;
}

void Program::__setstate__(std::vector<uint16_t> state) {
